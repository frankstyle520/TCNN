...
# -------辅助方法--------start
def insert_param(training, test, cnn_params, mmd_params, loop_size, data):
...
# MMD的超参
init_mmd_params = {'MMD_LAMBDA': 0.5, 'MMD_GAMMA': 10 ^ 3}
...
for path in path_train_and_test:
    ...
    for params in itertools.product(opt_n_filter, opt_filter_size, opt_n_hidden_node, opt_n_epoch):
        ...
        for l in range(LOOP_SIZE):
            ...
            for epoch in range(cnn_params['N_EPOCH']):
                ...
                for step, (batch_x, batch_y) in enumerate(loader):
                    ...
                    loss_mmd = mmd_loss(x_src_mmd, x_tar_mmd, mmd_params['MMD_GAMMA'])
        ...
        insert_param(training=path[0], test=path[1], cnn_params=cnn_params, mmd_params=mmd_params, loop_size=LOOP_SIZE, data=record_data)
        df = pd.DataFrame(data=record_data, columns=['Training Set', 'Test Set', 'Embedding Dim', 'Number of Filter', 'Filter Size',
                                                             'Number of Hidden Nodes', 'Learning Rate', 'Momentun', 'L2 Weight',
                                                             'Dropout', 'Number of Epoch', 'Stride', 'Padding', 'Batch Size', 'Pool Size', 'MMD Lambda', 'MMD Gamma', 'Loop Size',
